...
select a.*, c1.code_name as map_status_name, c2.code_name as map_prod_mode_name
from product_map a
left join code as c1 on a.map_status=c1.code_no and c1.code_type='MAP_STATUS'
left join code as c2 on a.map_mode=c2.code_no and c2.code_type='MAP_sMODE' 
";
...
where 1=1 ";
                     ...
                     if (!string.IsNullOrEmpty(_filter.cond))
                    {
                        ...
                        sqlParams.Add(_filter.argc);
                    }

                    return conn.QuerySingle<int>(sqlStmt, new { locale });
                }
            }
            catch (Exception ex)
            {
        ...
        public List<ProductMapExModel> GetMaps(string filter, string sorting, int size, int skip,
            string locale = null)
        {
            try
            {
...
where 1=1 ";
        ...
        private (string cond, dynamic argc) FilterParsing(string strJson)
        {
            var _filter = "";
            var _params = new ExpandoObject() as IDictionary<string, object>;

            if (string.IsNullOrEmpty(strJson)) return (_filter, _params);
...
values (:ota_prod_id, :ota_prod_name, :ota_plu, :map_prod_mode, :map_pkg_mode, :map_time_mode, :map_status, :prod_oid, :prod_name,
  ...
  allow_seltime, prod_oid, prod_name, pkg_oid, pkg_name, sku_id, sku_name, time_slots, create_user)
values (:ota_prod_id, :ota_prod_name, :ota_plu, :map_mode, :map_status, :allow_seltime, :prod_oid, :prod_name,
  :pkg_oid, :pkg_name, :sku_id, :sku_name, :time_slots::jsonb, :create_user) ";
    ...
    ota_plu=:ota_plu, map_prod_mode=:map_prod_mode, map_pkg_mode=:map_pkg_mode, map_time_mode=:map_time_mode, map_status=:map_status,
    prod_oid=:prod_oid, prod_name=:prod_name, pkg_oid=:pkg_oid, pkg_name=:pkg_name, item_oid=:item_oid,
    sku_id=:sku_id, sku_nae=:sku_nae, time_slots=:time_slots::jsonb, modify_user=:modify_user, modify_date=now()
