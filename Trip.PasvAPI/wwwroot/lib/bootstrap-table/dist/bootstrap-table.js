		...
		function () {
			...
			_createClass(BootstrapTable, [{
				...
				value: function initBody(fixedScroll) {
					...
					if (!hasTr) {
						var cells = this.$el.find('>thead>tr>th').length; // this.getVisibleFields().length;
						this.$body.html("<tr class=\"no-records-found\">".concat(Utils.sprintf('<td colspan="%s">%s</td>', cells, this.options.formatNoMatches()), "</tr>"));
					} else {
						if (!this.options.virtualScroll) {
				...
				value: function showLoading() {
					...
					if (this.options.loadingFontSize === 'auto') {
						fontSize = this.$tableLoading.width() * 0.016;
